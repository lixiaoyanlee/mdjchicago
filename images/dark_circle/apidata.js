...
const createApiData = (apiRet, divSize, _imgSize) => {
	const {dark_circle_info:{center_point,magnification,resize_points_dark_circle}={},align_img} = mockData;
	
	console.log(center_point,magnification,resize_points_dark_circle,'createApiData mockData')
	let pts = pointsData(resize_points_dark_circle);
	...
	let data = {
		...
		fragments: {
			...
			'dark_circle_img_l': {
				img:{w:(pts['2'].x-pts['1'].x),h:0,a:center_point},
				p:pts['1']
			},
			'dark_circle_img_r': {
				img:{w:(pts['6'].x-pts['5'].x),h:0,a:center_point},
				p:pts['5']
			},
			// 
			'dark_circle_twinkle':{
